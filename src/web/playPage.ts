...
function renderView(view: GameView, signed: GameState["artists"]): string {
  ...
  return `<div class="view-heading"><div><span class="eyebrow">Finance office</span><h1>Cash is creative oxygen.</h1><p>Runway matters more than vanity metrics. Every signing and hire adds a recurring commitment.</p></div></div><div class="metric-grid finance"><article><small>CASH</small><b>${formatMoney(state.cash)}</b></article><article><small>WEEKLY OVERHEAD</small><b>${formatMoney(weeklyCosts + staffCosts)}</b></article><article><small>TOTAL REVENUE</small><b>${formatMoney(state.totalRevenue)}</b></article><article><small>COMPANY VALUE</small><b>${formatMoney(state.companyValuation)}</b></article></div><section class="game-panel"><div class="panel-title"><div><span>CATALOG</span><h2>Revenue assets</h2></div></div>${state.songs.length ? `<div class="song-list">${state.songs.map((song) => `<article><div><span>${song.status}</span><b>${escapeHtml(song.title)}</b><small>Quality ${song.quality} · ${formatNumber(song.streams)} streams · ${formatNumber(song.radioSpins)} spins · Peak ${song.peakPosition ? `#${song.peakPosition}` : "—"}</small></div><em>${formatMoney(song.streams * 0.0035 + song.radioSpins * 2.1)}</em></article>`).join("")}</div>` : `<div class="mini-empty"><p>Your catalog has no recorded assets.</p></div>`}</section>`;
}
...
function bindGameEvents(): void {
  gameRoot().addEventListener("submit", async (event) => {
    ...
    engine = SimulationEngine.create({
      ...
      challengeId: String(data.get("challengeId") || "") || null
    });
