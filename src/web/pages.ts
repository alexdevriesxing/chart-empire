...
function legalBody(type: "privacy" | "terms"): string {
  if (type === "privacy") return `<div class="legal-copy"><h2>Local guest data</h2><p>Guest saves are stored in IndexedDB with a LocalStorage fallback on your device. Exported saves are downloaded directly by your browser.</p><h2>Cloud accounts and uploads</h2><p>When enabled, registered saves use Cloudflare D1 and uploaded logos use R2. Account export and deletion endpoints are included in the backend architecture.</p><h2>Consent and third parties</h2><p>Optional analytics, ads, personalized ads, and external embeds remain gated by your choices. YouTube may receive data only after its embed is loaded. Ad providers may process data according to their own terms when enabled.</p><h2>Contact data</h2><p>Contact submissions may include your name, email, message, source page, and Turnstile validation status. Do not submit sensitive personal information.</p></div>`;
